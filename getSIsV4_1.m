...
SIGGs=zeros(loopCount,2);
SIGGHs=zeros(loopCount,2);
...
GG=@(r,x) gammaSample(r,x);
    ...
    xHAdj=xH(1:end-1)+wH;
try
    mdlGG=fitnlm(xAdj,y,GG,SI);   
        rytov=mdlGG.Coefficients.Estimate;
    a=gammaAlpha(rytov);
    b=gammaBeta(rytov);
    SIGG=exp(a+b)-1;
        SIGGs(count,1)=SIGG;
    ...
    hold off
    ylabel('PDF');
legend("Observed sequence","Gamma Gamma SI="+SIGG);
xlabel('Intensity (Normalised to mean)');
    title(datestr(plotTime)+" Index: "+count)

    saveas(f,"G"+datestr(plotTime,30),'fig')
    saveas(f,"G"+datestr(plotTime,30),'png')
catch
    disp(count+"Failed")
end
try
    mdlGGH=fitnlm(xHAdj,y,GG,SIh);
        rytovH=mdlGGH.Coefficients.Estimate;
    aH=gammaAlpha(rytovH);
    bH=gammaBeta(rytovH);
    
            SIGGHs(count,1)=SIGGH;
    SIGGHs(count,2)=count;
    SIGGH=exp(aH+bH)-1;
    plot(xHAdj,yH)
    hold on
    line(xHAdj.',predict(mdlGGH,xHAdj.'),'Color','r');
    hold off
    title(datestr(plotTime)+" Index: "+count+"H")
    ylabel('PDF');
legend("Observed sequence","Gamma Gamma SI="+SIGGH);
xlabel('Intensity (Normalised to mean)');
    saveas(f,"GH"+datestr(plotTime,30),'fig')
    saveas(f,"GH"+datestr(plotTime,30),'png')
catch
        disp(count+"FailedH")
...
%                         errSeqsH(count)=mat2cell(errSeqnfH.',1);
    ...
    save("GESI_Ts"+datestr(startTime,30),"SIs","SIHs","SIGGs","SIGGHs","BERS","BERSh","totalHours","timeToRead",'-v7.3');
