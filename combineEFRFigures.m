clear all
clc
addpath '..\..\data'
addpath '..\..\Figures\Paper'
addpath '..\..\Figures\MOEA'

 fh1 = openfig('EFRA.fig','invisible' );
 fh2 = openfig('EFRB.fig','invisible' ); 
 fh3 = openfig('EFRC.fig','invisible' );
 fh4 = openfig('EFRD.fig','invisible' );

fh1.Children.Children;
 fh1Child=ans;
 fh2.Children.Children;
  fh2Child=ans;
  fh3.Children.Children;
 fh3Child=ans;
   fh4.Children.Children;
 fh4Child=ans;
newcolors = {'#F00','#F80','#FF0','#0B0','#00F','#50F','#A0F','#000','#BBB'};
newcolors =[0.4660, 0.6740, 0.1880;0.3010, 0.7450, 0.9330;0.4940, 0.1840, 0.5560 ;]

%defaultColors=[0 0.4470 0.7410;0.8500 0.3250 0.0980;0.9290 0.6940 0.1250 ;0.4940 0.1840 0.5560;0.4660 0.6740 0.1880;0.3010 0.7450 0.9330;0.6350 0.0780 0.1840;]
markers=["none" "none" "*" "none" "none" "*" "none" "none" "*"];
lineStyles=[  ":" ":" ":"  "--" "--" "--" "-." "-." "-." "-" "-" "-"];
lineWidths=[ 3.2];

combFig=figure();
ax = axes(combFig);
%colororder(
hold on

copyobj(fh1Child,ax);
copyobj(fh2Child,ax);
copyobj(fh3Child,ax);
copyobj(fh4Child,ax);

currAxis=gca;
set(currAxis,'xscale','log')
h=get(currAxis,'Children')
legendString= ["A-" "A-" "A-" "B-" "B-" "B-" "C-" "C-" "C-" "D-" "D-" "D-"];
for count =1:12
    legendString(count)=strcat(legendString(count),h(13-count).DisplayName);
    h(count).LineStyle=lineStyles(count);
    h(count).LineWidth=lineWidths(1);
    h(count).Color=newcolors(mod(count-1,3)+1,:)
end
%set(currAxis,'yscale','log')
legend(legendString);
   ylabel('Pr(0^{m}|1)')
xlabel("length of interval (m)")
close(fh1);
close(fh2);
close(fh3);
close(fh4);