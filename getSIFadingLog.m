...
for count=1:loopCount
   ...
   f=figure;
    ...
    rms=sqrt(rytov)
          %  plot(xAdj,y)
        h=histogram((log(tailored)-expectedValue(log(tailored)))/rms,nBins,'Normalization','pdf','DisplayStyle','stairs');
    x=h.BinEdges;
    w=h.BinWidth;
    xAdj1=x(1:end-1)+w;
    plot(xAdj1,y*rms)
    ...
    hold on
    line(xAdj1,predict(mdlLn,xAdj.')*rms,'Color','r');
   ...
   % set(gca, 'XScale', 'log')
       ...
       legend("Observed sequence","Log Normal Turbulence Rytov="+rytov);
